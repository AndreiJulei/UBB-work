...
(defun find-in-children (rest child-count x level)
  (cond
    ...
    (t
      (let (
             (result (find-node rest x (+ level 1)))
           )
        ...
        (if (car result)
            ...
            ;; Not found -> move past subtree and continue
            (find-in-children
              (cdr result)
              (- child-count 1)
              x
...
% levelOfNode(Tree, X) = result_level, where (result_level, _) = findNode(Tree, X, 0)

% findNode([Node, Count | Rest], X, Level) = 
%   | (Level, Rest), if Node = X;
%   | findInChildren(Rest, Count, X, Level + 1), if Node != X.

% findInChildren(Rest, ChildCount, X, Level) = 
%   | (nil, Rest), if ChildCount = 0;
%   | (FoundLevel, Remainder), if findNode(Rest, X, Level) returns (FoundLevel, Remainder) and FoundLevel != nil;
%   | findInChildren(Remainder, ChildCount - 1, X, Level), otherwise.