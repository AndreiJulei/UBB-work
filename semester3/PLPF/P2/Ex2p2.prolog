% a. Sort a list with keeping double values in resulted list. E.g.: [4 2 6 2 3 4] --> [2 2 3 4 4 6]
% b. For a heterogeneous list, formed from integer numbers and list of numbers, write a predicate to sort every
% sublist, keeping the doubles.
% Eg.: [1, 2, [4, 1, 4], 3, 6, [7, 10, 1, 3, 9], 5, [1, 1, 1], 7] =>
% [1, 2, [1, 4, 4], 3, 6, [1, 3, 7, 9, 10], 5, [1, 1, 1], 7].

% a.manualSort(List,Resutl)

% insert(Element, List, Result) - insert Element into sorted List
insert(X, [], [X]).
insert(X, [H|T], [X,H|T]) :- 
    X =< H.

insert(X, [H|T], [H|R]) :-
    X > H,
    insert(X, T, R).

% manualSort(List, Sorted)
manualSort([], []).
manualSort([H|T], Sorted) :-
    manualSort(T, SortedTail),
    insert(H, SortedTail, Sorted).

% b. processList(List,Resutl)

processList([], []).

processList([H|T], [RH|RT]) :-
    is_list(H),              % if H is a sublist
    manualSort(H, RH),             % sort + remove duplicates
    processList(T, RT).

processList([H|T], [H|RT]) :-   % if H is a number
    \+ is_list(H),
    processList(T, RT).
